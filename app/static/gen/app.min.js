...
body.events.forEach(function(e){e.startdate=moment(e.startdate).lang('nb');e.enddate=moment(e.enddate).lang('nb');});var eventDate=body.events[0].startdate;var eventEndDate=body.events[0].enddate;console.log(eventEndDate);console.log(eventDate);var now=moment();if(eventEndDate.isBefore(now)||eventDate.isSame(now,'day')){body.today=body.events.shift()||null;console.log(body.today);}else{body.today=null;}
...
var rows=c.data.events.map(function(event){return m('tr',[m('td',{'class':'fade summary'},jrvs.truncate(event.summary,19)),m('td.start',event.startdate.format('DD. MM HH:mm'))]);});return[m('p.fade','Creativia Scenen:'),m('h1',c.data.today?c.data.today.date.format('HH:mm'):'--:--'),m('h2',c.data.today?jrvs.truncate(c.data.today.summary,20):'Ingenting skjer!'),m('table',rows),m('p',{'class':'fade updated-at'},'Sist oppdatert: '+
